...
class TaskListItem extends React.Component {
  ...
  checkForCommonErrors(lines) {
    for (let line of lines) {
      if (
        ...
        this.setState({
          friendlyTaskError: `It looks like there might be one of the following problems:
        <ul>
  ...
  getRestartSubmenuItems() {
    ...
    PipelineSteps.get()
      .slice(1)
      .forEach((rf) => (rfMap[rf.action] = rf));

    // Create onClick handlers
    for (let rfParam in rfMap) {
      rfMap[rfParam].label = "Desde " + rfMap[rfParam].label;
